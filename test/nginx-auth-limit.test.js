'use strict';
// deploy/nginx/openvibe.codes.conf: the shared navbar asks /auth/me on every page view, so the session probe has its own location on
...
const conf = fs.readFileSync(path.join(__dirname, '..', ...'deploy/nginx/openvibe.codes.conf'.split('/')), 'utf8');
...
assert.match(probe, /limit_req zone=ovcodes_api burst=30 nodelay;/);
assert.match(auth, /limit_req zone=ovcodes_auth burst=10 nodelay;/);
